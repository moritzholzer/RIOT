...
static void my_set_confirm(void *arg, uint8_t handle, int status, ieee802154_pib_attr_t attr) {
    (void)arg; (void)handle; (void) attr;
    printf("SET confirm res=%d (%s)\n", status, strerror(-status));
}
static void my_get_confirm(void *arg, uint8_t handle, int status, ieee802154_pib_attr_t attr, ieee802154_pib_value_t value) {
    (void)arg; (void)handle; (void) attr;
    printf("GET confirm res=%d (%s) with value= %hu\n", status, strerror(-status), value.v.u8);
}
...
int main(void){
    ieee802154_mac_cbs_t cbs = {
        ...
        .mlme_set_confirm = my_set_confirm,
        .mlme_get_confirm = my_get_confirm,
        .arg = NULL,
    };
    ...
    int res = ieee802154_mac_mlme_set_request(&mac, IEEE802154_PIB_EXTENDED_ADDRESS, &pib_value);
    if (res != 0) {
        printf("set failed: %d\n", res);
        return 1;
    }
    ...
    res = ieee802154_mac_mlme_set_request(&mac, IEEE802154_PIB_SHORT_ADDR, &pib_value);
    if (res != 0) {
        printf("set failed: %d\n", res);
        return 1;
    }
    ...
    res = ieee802154_mac_mlme_get_request(&mac, IEEE802154_PIB_DSN);
    if (res != 0) {
        printf("get failed: %d\n", res);
        return 1;
    }
